...
信息中的 XML 生成器转换是一种主动的、连接的转换。这种转换在管道内生成 XML 文件时非常有用。
...
一旦你点击了创建..选项，创建[变换](https://www.tutorialgateway.org/informatica-transformations/)窗口打开。请从下拉列表中选择 XML 生成器转换，并指定唯一名称(XML_GenCustomer)，然后单击创建按钮
...
非 XML 源:如果您没有 XSD 文件，并且想要使用信息源定义中的 SQL 表，则选择此选项。在本例中，我们将使用源定义中的客户记录表。
