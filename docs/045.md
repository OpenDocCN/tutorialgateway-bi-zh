...
信息论中的更新策略转换是一种主动的、关联的转换。基于源表数据更新、插入或删除目标中的记录非常有用。这种转换的好处是:您可以限制记录不到达目标表。
...
将客户记录源定义从“源”文件夹拖放到映射设计器中。拖动源后，动力中心设计师会自动为您创建[源限定符变换](https://www.tutorialgateway.org/source-qualifier-transformation-in-informatica/)。
