...
### 在Informatica 目标定义中创建前 SQL 和后 SQL
...
双击源限定符转换，并转到属性选项卡以配置Informatica 前 SQL 和后 SQL
...
### 创建Informatica 预 SQL 和后 SQL 工作流

在我们完成创建Informatica 预 SQL 和后 SQL 映射后，我们必须为其创建工作流。电源中心工作流管理器提供了两种创建工作流的方法。
...
现在，让我开始Informatica 预 SQL 和后 SQL 工作流。为此，请导航到“工作流”菜单并选择“启动工作流”选项。
