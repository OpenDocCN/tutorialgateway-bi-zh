...
如何用例子格式化 Power BI 带状图？。格式化 Power BI 带状图包括更改功能区颜色、带状图标题文本、标题位置等。

为了演示 Power BI 带状图式选项，我们将使用之前创建的带状图。请参考 Power BI 文章中的[带状图，了解创建](https://www.tutorialgateway.org/create-a-ribbon-chart-in-power-bi/) [Power BI](https://www.tutorialgateway.org/power-bi-tutorial/) 带状图的步骤。

## 如何格式化 PowerBI 带状图

请单击“格式”按钮查看此带状图可用的格式选项列表。
...
以下是可用于格式化带状图水平轴或 X 轴的选项列表。从下面的截图中可以看到，我们将颜色改为棕色，字体样式改为 Candara，文本大小改为 20。

![Format Power BI Ribbon Chart 4](img/10bf383db21f4f97ad347fd699377dad.png)

默认情况下，带状图 X 轴标题设置为关闭，但您可以通过将标题切换为打开来启用它。让我将标题颜色更改为绿色，字体样式更改为红色，文本大小更改为 30。

![Format Power BI Ribbon Chart 5](img/268b54e6e35f4196e53ac137c0b64891.png)

### 格式化带状图数据颜色

默认情况下，带状图将为所有功能区选择默认颜色。但是，您可以根据自己的要求进行更改。
...
带状图数据标签在条形图中显示指标值(每个组的销售额)。从下面的截图中可以看到，我们为此带状图启用了数据标签，并将颜色更改为白色。

![Format Power BI Ribbon Chart 8](img/d9b675c8b9d04dbdae04ddc7cf44b39a.png)

### 格式化带状图绘图区

使用此绘图区域属性，可以添加自定义图像作为带状图的背景。出于演示目的，我们添加了一个图像作为绘图区域背景。
...
### 用 PowerBI 背景色和边框格式化带状图

通过将“背景”选项切换为“开”，可以将背景颜色添加到带状图中。为了演示带状图，我们添加了透明度为 38%的浅黄色。

同样，您可以通过将“边框”选项从“关闭”切换到“打开”来向带状图添加边框。
