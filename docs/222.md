...
接下来的例子将向您展示[表格](https://www.tutorialgateway.org/tableau/)中的字符串函数列表
...
让我将此字段添加到现有表中。请参考[创建表报告](https://www.tutorialgateway.org/tableau-table-report/)一文，了解
...
Tableau Trim 函数是字符串函数之一，它从字符串的左侧和右侧移除空格。这个表格修剪函数的语法是:
