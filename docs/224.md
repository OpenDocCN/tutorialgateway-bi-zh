...
Tableau 提供了各种类型转换函数来对我们的数据执行类型转换。它们是Tableau 日期、日期时间、日期解析、整数、浮点、SRT 等。在本文中，我们通过示例展示了如何使用 Tableau 转换函数。
...
Tableau Date 函数从给定的表达式、数字或字符串中返回日期。这个Tableau 日期函数的语法是:
